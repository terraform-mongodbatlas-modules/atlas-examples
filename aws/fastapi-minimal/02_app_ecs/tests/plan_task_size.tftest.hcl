...
run "task_size_from_handoff" {
  ...
  assert {
    condition = alltrue([
      module.app.task_cpu == "1024",
      module.app.task_memory == "2048",
      module.app.container_port == 8001,
    ])
    error_message = "Handoff task_cpu/task_memory should flow to module outputs"
  }
}
