...
- **`http_edges`:** Map of ALB + CloudFront edges. Public subnets are created for those regions. WAF is on by default; set `waf.disabled = true` to skip. `waf.common_rule_set_count_rules` counts named Common Rule Set rules (empty by default).
