...
- **`mongo`:** `connection_string`, `database_name`. Sets `MONGO_URL` and `DB_NAME`.
...
The example `app/` root reads the SM secret and passes these groups. The module does not call Secrets Manager.
